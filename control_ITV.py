...
def refresh():
    ...
    for j in Cl_list:
        ...
        frame.grid(row=1, column=count)
        color = "red"
        if j != 1: #"green" means connected, "red" means not connected
            color = "green"
        label = tk.Label(master=frame, text=f"ITV {count + 1}", background=color)
        count += 1
        label['font'] = myFont
        label.pack()
...
# define font
myFont = font.Font(size=40) #size of text
window.geometry("480x320") #window size
...
btn_refresh.grid(row=2, column=1, sticky="nsew")
...
btn_quit.grid(row=2, column=0, sticky="nsew")
